...
class SubclusterAnalysis:
    def __init__(self, ad, save_path, cluster_key='leiden_Raw_res1'):
        ...
        self.new_colors = self._generate_colors(200)
    ...
    def _ward_linkage(self, X, y):
        centroids, unique_labels = self._calculate_centroids(X, y)
    ...
    def calculate_metrics(self, nonepi=True, plot_spatial=True):
        ...
        plt.savefig(self.save_path + "/umap_clusters.pdf", bbox_inches='tight', dpi=600)
        plt.show()
        plt.close()
        ...
        if plot_spatial:
            ...
            for collection in ax.collections:
                collection.set_rasterized(True)
            plt.tight_layout()
            plt.savefig(self.save_path + "/spatial_clusters.pdf", bbox_inches='tight', dpi=600)
            plt.show()
            plt.close()
        ...
        fig, axs = plt.subplots(1, 2, figsize=(9, 3), dpi=600)  # 调整画布大小以适应布局
        ...
        plt.tight_layout()
        plt.savefig(self.save_path + "/var_plot_withk.pdf", dpi=600)
        plt.show()
        plt.close()
    ...
    def kde_plot(self):
        ...
        fig, axes = plt.subplots(int((len(self.umap_keys)+2)/3), 3, figsize=(9, len(self.umap_keys)), dpi=600)
        ...
        for key in self.umap_keys:
            ...
            ax.set_title(key)
        plt.tight_layout()
        plt.savefig(self.save_path + "/umaps_kde.pdf", dpi=600)
        plt.show()
        plt.close()
    ...
    def detect_subclusters(self, n_clusters=2, plot_all_scales=True, calculate_ward=True, plot_spatial=True):
        ...
        plt.savefig(self.save_path + "/umap_subclusters_scale" + str(self.optimal_scale) + ".pdf", dpi=600)
        plt.show()
        plt.close()
        ...
        if calculate_ward:
            ...
            for key in umap_keys:
                ...
                ward_list[key] = self._ward_linkage(X, y)

        if plot_all_scales:
            fig, axes = plt.subplots(int((len(umap_keys)+2)/3), 3, figsize=(9, len(umap_keys)), dpi=600)
            ...
            plt.tight_layout()
            plt.savefig(self.save_path + "/umaps_cluster_scale" + str(self.optimal_scale) + ".pdf", dpi=600)
            plt.show()
            plt.close()
        ...
        if plot_spatial:
            ...
            for collection in ax.collections:
                collection.set_rasterized(True)
            plt.tight_layout()
            plt.savefig(self.save_path + "/spatial_" + str(self.selected_cluster) + "_scale" + str(self.optimal_scale) + ".pdf", dpi=600)
            plt.show()
            plt.close()
    ...
    def deg_analysis_subclusters(self, dotplot=True):
        ...
        if dotplot:
            ...
            plt.savefig(self.save_path + "/dotplot_degenes.pdf", dpi=600)
            plt.show()
            plt.close()

    def _getKNN(self, adata, k):
    ...
    def _calculate_neighbor_composition(self, knn_matrix, clusters, num_clusters):
    ...
    def _subcluster_label_merge(self):
        celltype_labels = self.ad.obs[self.cluster_key]
        self.ad.obs['celltype_sub'] = [celltype_labels[i] if i not in self.ad_selected.obs_names else (celltype_labels[i] + '_cluster' + self.ad_selected.obs['cluster_label'][i]) for i in celltype_labels.index]
        print("Saving cell type labels with subcluster labels...")
        pd.DataFrame({'cell_id': self.ad.obs['cell_id'], 'group': self.ad.obs['celltype_sub']}).to_csv(self.save_path + "/cell_groups.csv", index=False)
        ad_plot = self.ad[self.ad.obs['celltype_sub'] != self.selected_cluster+'_cluster-1']
        return ad_plot
    ...
    def neighborhood_analysis(self, k=10):
        ...
        ad_plot = self._subcluster_label_merge()
        celltype_labels = ad_plot.obs['celltype_sub']
        n_cluster = len(np.unique(celltype_labels))
        knn_similarity_matrix = self._getKNN(ad_plot, k=10)
        ...
        # 计算邻居组成比例
        component_matrix = self._calculate_neighbor_composition(
    ...
    def morphology_analysis(self, morphology_df):
        ...
        for key in keys:
            ...
            p_value = self._calculate_p_value(group0, group1)
        ...
        fig, axes = plt.subplots(n_row, 3, figsize=(10, 3*n_row), dpi=600)
        ...
        plt.tight_layout()
        plt.savefig(self.save_path + f"/morphology_boxplot_{key}.pdf", bbox_inches='tight', dpi=600)
        plt.show()
        plt.close()
